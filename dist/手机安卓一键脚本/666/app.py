...
class ConfigManager:
    ...
    def load_config(self):
        try:
            ...
            log_and_print_error(e, "请检查 config.ini 文件的格式是否正确。例如，确保每个部分都有 `[SECTION_NAME]` 这样的标题。")
            sys.exit(1)
    def create_default_config(self):
        ...
        self.config['API_KEYS'] = {'group1': json.dumps(["请在此处填入您的API密钥1"]), 'group2': json.dumps(["请在此处填入您的API密钥2"])}
    ...
    def get_api_keys(self):
        try: conf = self.get_config_section('API_KEYS'); return {k: json.loads(v) for k, v in conf.items()}
        except json.JSONDecodeError as e: log_and_print_error(e, "config.ini 中 [API_KEYS] 的值不是有效的JSON格式。"); return {'group1': [], 'group2': []}
...
if FASTAPI_AVAILABLE:
    ...
    app_fastapi = FastAPI(title="LLM代理服务", version="2.3-fixed-zh")
    ...
    async def send_single_request(client: httpx.AsyncClient, api_key: str, data: dict) -> Tuple[Any, Any]:
        ...
        try:
            resp = await client.post(url, headers=headers, json=data, timeout=config_manager.get_server_config()['request_timeout'])
            resp.raise_for_status()
            # 在解析JSON前先检查内容是否为空
            if not resp.content:
                logger.warning(f"API请求成功 (HTTP {resp.status_code}) 但响应体为空。Key: ...{api_key[-4:]}。")
                return None, ("bad_response", "响应体为空")
            return resp.json(), None
        except json.JSONDecodeError:
            logger.warning(f"JSON解析失败 (HTTP {resp.status_code})。API可能返回了非JSON内容（如HTML错误页）。Key: ...{api_key[-4:]}。")
            return None, ("bad_response", "响应非JSON格式")
        except httpx.HTTPStatusError as e:
            status = e.response.status_code; error_type = "auth" if status in [401, 403] else "rate_limit" if status == 429 else "other"
            logger.warning(f"API请求失败 (HTTP {status}) 使用 key: ...{api_key[-4:]}。")
            return None, (error_type, f"HTTP {status}")
        except httpx.RequestError as e: # 包括 ReadError, ConnectError 等
            logger.warning(f"网络请求失败使用 key: ...{api_key[-4:]}。原因: {e.__class__.__name__}")
            return None, ("timeout", str(e))
        except Exception as e:
            log_and_print_error(e, "发送单个API请求时发生未知内部错误。")
    ...
    async def process_api_requests(request_data: dict) -> Dict:
        ...
        valid_responses = []
        errors = {"auth": 0, "rate_limit": 0, "timeout": 0, "bad_response": 0, "other": 0}
        ...
        # 更新错误报告，加入bad_response
        error_report = f"失败统计: {errors['auth']}个认证失败, {errors['rate_limit']}个速率限制, {errors['timeout']}个连接超时, {errors['bad_response']}个无效响应, {errors['other']}个其他错误。"
        raise HTTPException(503, {"error": "多个上游API请求失败。", "report": error_report})
    ...
    # ==================== Web UI 和配置 API ====================
    # (此部分代码无变化，保持不变)
    static_path = get_resource_path("static"); templates_path = get_resource_path("templates")
    ...
    async def read_root_html(request: Request):
        ...
        if not index_path.exists(): return HTMLResponse("<h1>错误：未找到界面文件</h1><p>请确保 'templates/index.html' 文件与您的Python脚本在同一目录下。</p>", status_code=404)
...
def main():
    ...
    try:
        ...
        if not check_port_available(web_host, web_port):
            ...
            else: print(f"[处理办法]：请关闭占用端口 {web_port} 的程序，或在 config.ini 中修改 'port'。"); sys.exit(1)
        ...
        log_and_print_error(e, "启动服务时发生严重错误，程序已退出。")
        sys.exit(1)
